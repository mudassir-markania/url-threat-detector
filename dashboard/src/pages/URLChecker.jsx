...
import { Search, Shield, AlertTriangle, CheckCircle, Loader, Globe, Clock, Tag } from 'lucide-react'

// Score color + label helper
function getScoreInfo(score) {
  if (score >= 80) return { color: '#ef4444', label: 'Critical Threat',  glow: 'glow-red',   icon: AlertTriangle }
  if (score >= 50) return { color: '#f59e0b', label: 'Suspicious',       glow: 'glow-yellow', icon: AlertTriangle }
  return               { color: '#10b981', label: 'Safe',              glow: 'glow-green',  icon: CheckCircle }
}
...
function ScoreMeter({ score }) {
  ...
  return (
    <div className={`relative flex items-center justify-center w-40 h-40 ${glow}`} style={{ borderRadius: '50%' }}>
...
function URLChecker() {
  ...
  const handleCheck = async () => {
    ...
    // ── SIMULATED API CALL ──────────────────────────────────────
    // In Phase 6, this will be replaced with:
    //   const response = await axios.post('/api/score-url', { url })
    //   setResult(response.data)
    await new Promise((res) => setTimeout(res, 1800)) // fake delay
    const fakeScore = Math.floor(Math.random() * 100)
    setResult({
      url,
      score:    fakeScore,
      category: fakeScore >= 80 ? 'Malware' : fakeScore >= 50 ? 'Suspicious' : 'Clean',
      source:   'URLhaus Feed',
      ...
      <div className="max-w-2xl mx-auto">
        ...
        >
          <div className="flex gap-3">
            <div className="flex-1 flex items-center gap-3 px-4 py-3 rounded-lg" style={{ background: 'rgba(255,255,255,0.04)' }}>
              <Globe size={16} style={{ color: 'rgba(255,255,255,0.3)', flexShrink: 0 }} />
              <input
                type="text"
                value={url}
          ...
          {error && (
            <p className="mt-3 text-xs px-1" style={{ color: '#ef4444' }}>{error}</p>
          )}
        </motion.div>
              ...
              <div className="space-y-3">
                {[
                  ...
                  return (
                    ...
                    >
                      <Icon size={15} className="mt-0.5 shrink-0" style={{ color: 'rgba(255,255,255,0.3)' }} />
                      <div className="min-w-0">
                        <p className="text-xs mb-0.5" style={{ color: 'rgba(255,255,255,0.35)' }}>{row.label}</p>
                        <p className="text-sm font-medium text-white break-all">{row.value}</p>
                      </div>
                    </div>
                  )
                })}
